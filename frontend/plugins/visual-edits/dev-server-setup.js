      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="choosepure-team" -c user.email="contact@choosepure.in" add "${targetFile}"`);
            execSync(`git -c user.name="choosepure-team" -c user.email="contact@choosepure.in" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
